...
public class FontFile extends AssetFile{
    public static Seq<FontFile> all = new Seq<>();

    public Font font;

    public FontFile(String name){
        super(name);
        font = new Font();
        all.add(this);
    }
    ...
    @Override
    public FontFile load(){
        return (FontFile)super.load();
    }

    @Override
    public FontFile create(){
        super.create();
        try{
            ...
            for(int i = 0;i < font.pages;i++){
                ...
                font.page[id] = new TextureFile(parent() + "/" + name).load().texture;
            }
